# GitHub upload notes for Readwide 1.0.14
...
- `docs/GITHUB_RELEASE_NOTES_READWIDE_1_0_14.md` (and the retained per-version notes back through 1.0.2)
...
1. Tag the exact release commit as `v1.0.14` and push the tag.
2. Start from the current fdroiddata upstream metadata. It already contains Readwide builds through 1.0.12; add only the 1.0.14 build block, set its `commit` field to the full 40-character commit hash that the `v1.0.14` tag points to, then update `CurrentVersion` to `1.0.14` and `CurrentVersionCode` to `10014`. This app's F-Droid maintainer requires a full commit hash rather than the tag name.
