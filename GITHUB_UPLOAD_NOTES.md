# GitHub upload notes for Readwide 1.0.11
...
The `applicationId` has been `com.readwide.manager` since 1.0.4. 1.0.11 keeps the `readwide` release signing key introduced in 1.0.6, so it installs in place over 1.0.10, 1.0.9, 1.0.8, 1.0.7, and 1.0.6. Updating from 1.0.4/1.0.5 (which used the previous key) still requires uninstalling the old version, installing 1.0.11, and transferring data with the in-app JSON backup export/import, because of the 1.0.6 signing-key change. Older `com.textview.reader` builds also install as a separate app and migrate the same way.
...
1. Tag the exact release commit as `v1.0.11` and push the tag.
2. Set the `1.0.11` build entry's `commit` field in `fdroid/metadata/com.readwide.manager.yml` to the full 40-character commit hash that the `v1.0.11` tag points to. This app's F-Droid maintainer requires a full commit hash (not a tag name), so use the hash directly rather than the tag. The earlier entries through 1.0.10 already carry their real release commit hashes.
