# GitHub upload notes for Readwide 1.0.19
...
The `applicationId` has been `com.readwide.manager` since 1.0.4. 1.0.19 keeps the `readwide` release signing key introduced in 1.0.6, so it installs in place over 1.0.18 through 1.0.6. Updating from 1.0.4/1.0.5 (which used the previous key) still requires uninstalling the old version, installing 1.0.19, and transferring data with the in-app JSON backup export/import, because of the 1.0.6 signing-key change. Older `com.textview.reader` builds also install as a separate app and migrate the same way.
...
1. Tag the exact release commit as `v1.0.19` and push the tag.
2. Start from the current fdroiddata upstream metadata; the checked-in historical mirror stops at 1.0.13 and must not be copied over upstream. Add only the 1.0.19 build block, set its `commit` field to the full 40-character commit hash that the `v1.0.19` tag points to, then update `CurrentVersion` to `1.0.19` and `CurrentVersionCode` to `10019`.
...
Run the build, test, lint and APK checks above on the release commit. Publish the source ZIP and APK SHA-256 checksums with the release assets.
