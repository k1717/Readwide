...
final class ReaderDialogStyleController {
    ...
    android.app.Dialog createPositionedReaderDialog(@NonNull View content,
                                                            ...
                                                            float widthFraction,
                                                            boolean adjustResize) {
        // Only one positioned reader dialog should be visible at a time. Dismiss
        // any previous one so rapid or repeated toolbar taps cannot stack dialogs.
        if (currentPositionedDialog != null) {
            try {
                if (currentPositionedDialog.isShowing()) currentPositionedDialog.dismiss();
            } catch (Exception ignored) {
            }
            currentPositionedDialog = null;
        }
        ...
        // OnDismissListener here because several callers set their own (only one
        // is allowed); the isShowing() guard above safely handles a stale, already
        // dismissed reference when the next dialog opens.
        currentPositionedDialog = dialog;
        return dialog;
    }
