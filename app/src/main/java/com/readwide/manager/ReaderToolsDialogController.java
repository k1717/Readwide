...
final class ReaderToolsDialogController {
    ...
    void showTextSearch() {
        ...
        // Option 2: a compact find-options row (case / whole-word / regex).
        // Unicode normalization is always applied by the matcher and is not a toggle.
        final CheckBox caseBox = makeSearchOptionCheck(activity.getString(R.string.search_option_case_sensitive), fg,
                activity.prefs != null && activity.prefs.getReaderSearchCaseSensitive());
        final CheckBox wholeBox = makeSearchOptionCheck(activity.getString(R.string.search_option_whole_word), fg,
                activity.prefs != null && activity.prefs.getReaderSearchWholeWord());
        final CheckBox regexBox = makeSearchOptionCheck(activity.getString(R.string.search_option_regex), fg,
        ...
        Runnable recount = () -> {
            ...
            if (activity.largeTextEstimateActive) {
                ...
                // is recomputed under the new options, and show a neutral count.
        ...
        optionsRow.setPadding(0, 0, 0, activity.dpToPx(4));
    ...
    private CheckBox makeSearchOptionCheck(String label, int fg, boolean checked) {
        ...
        cb.setMinHeight(activity.dpToPx(40));
        cb.setPadding(cb.getPaddingLeft(), activity.dpToPx(4), cb.getPaddingRight(), activity.dpToPx(4));
        // Tint the box to match dialog text so it reads on dark reader themes.
        androidx.core.widget.CompoundButtonCompat.setButtonTintList(
                cb, android.content.res.ColorStateList.valueOf(fg));
        return cb;
    }
    ...
    private TextView makeSearchDialogButton(String label, int bg, int fg, int sub) {
        ...
        // Wrap to multiple lines for long translations (e.g. German) instead of
        // clipping; keep a tap-target minimum height for short labels.
        button.setMinHeight(activity.dpToPx(44));
        button.setPadding(activity.dpToPx(4), activity.dpToPx(8), activity.dpToPx(4), activity.dpToPx(8));
