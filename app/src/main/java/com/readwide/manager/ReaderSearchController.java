...
final class ReaderSearchController {
    ...
    LargeTextSearchResult searchCurrentLargeTextPartitionInstant(@NonNull String query,
        ...
        com.readwide.manager.util.SearchMatcher matcher =
                com.readwide.manager.util.SearchMatcher.compile(query, activity.currentSearchOptions());
        if (matcher == null) return null;
        int idx = -1;

        if (forward) {
            int from = Math.max(0, Math.min(bodyEnd, localStart));
            com.readwide.manager.util.SearchMatcher.Match hit = matcher.firstFrom(activity.fileContent, from);
            if (hit != null && hit.end <= bodyEnd) {
                idx = hit.start;
            }
        } else {
            ...
            while (hit != null && hit.end > bodyEnd) {
                ...
                hit = matcher.lastUpTo(activity.fileContent, hit.start - 1);
            }
            if (hit != null) {
                idx = hit.start;
            }
