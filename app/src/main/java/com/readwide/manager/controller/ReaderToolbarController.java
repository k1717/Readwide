package com.readwide.manager.controller;
...
public final class ReaderToolbarController {
    ...
    // Guards against a toolbar action running multiple times when a button is
    // tapped repeatedly (which previously opened duplicate dialogs / reloaded).
    private long lastToolbarActionAt = 0L;
    private static final long TOOLBAR_ACTION_MIN_INTERVAL_MS = 350L;
    ...
    private void configureBasicButton(@NonNull View button, @Nullable Runnable action) {
        ...
        button.setOnClickListener(v -> {
            ...
            if (now - lastToolbarActionAt < TOOLBAR_ACTION_MIN_INTERVAL_MS) return;
