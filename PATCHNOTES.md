...
- FOSS/provenance at that intermediate stage: PPMd/BCJ2 decoding stayed inside the then-shipped BSD-licensed libarchive binary; its PPMd is Igor Pavlov's public-domain `Ppmd7.c` (based on Dmitry Shkarin's public-domain PPMd var.H). No 7-Zip code was copied and no new dependency was added by that change. The current 1.0.17 build supersedes only the packaging detail by compiling pinned libarchive 3.8.9 source from `third_party/libarchive-android`.
