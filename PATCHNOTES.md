...
- Decoding wires into the existing dedicated readers: `ZstdCompressorInputStream` (Commons Compress over the already-bundled zstd-jni, until now used only for ZIP method-93 entries) and `FramedLZ4CompressorInputStream` (pure Java, standard LZ4 frame magic `04 22 4D 18`). Both are added to `wrapTarPayloadInputStream` and `wrapSingleCompressedInputStream`, and the new tar types join every tar-family switch: password probing, entry listing, single-entry and whole-archive extraction, the forward sequential reader (`ArchiveImageEntryCache` treats them as sequential like the other compressed tars), and the generic libarchive fallback (the bundled native libarchive links liblz4 and libzstd, so the fallback covers these formats too).
- `ZstdLz4ArchiveSupportTest` embeds real fixtures - a two-entry USTAR tar compressed as a zstd frame and as an LZ4 frame, plus single-file forms - and runs list + extract end to end through `ArchiveSupport`, verifying decompressed content byte-for-byte and the name-detection edge cases. zstd-jni ships desktop natives, so the zstd path executes in plain JVM unit tests.
...
- **RAR5 `-hp` is a hard unsupported boundary, not a libarchive delegation.** bsdtar/libarchive 3.7.2 cannot even list a real WinRAR 7.00 header-encrypted RAR5 archive ("Encryption is not supported"), and no first-party RAR5 header decryptor exists. `RarHeaderEncryptionDetector` gained `headerEncryptedRarVersion` so the unsupported message now distinguishes RAR5 `-hp` (no decrypt path in either backend) from RAR4 `-hp` (a first-party header rewriter exists, so a failure there names the rewriter/libarchive). Detector tests cover both versions plus the non-encrypted zero case. The previous Javadoc/doc claim that "RAR5 header decryption still belongs to libarchive" was wrong and is corrected.
...
- ZipCrypto-encrypted EGG entries (EncryptMethod 0) now decrypt. The ENCRYPT extra field carries `method u8 + check data [12] + crc u32`; the reader builds one `AlzipZipCrypto` per file, verifies `plain[11] == crc >> 24` on the check data before writing anything (wrong password -> `BAD_PASSWORD`, no partial file), and continues the same keystream across the file's blocks - block boundaries must not reset the cipher (verified against a real two-block file where a reset corrupts block 2). The stored payload is ciphertext end to end, so `writeBlock` was restructured around a single sequential per-block stream (bounded, optionally decrypting): the 9-byte LZMA preamble and the AZO frame are consumed from that stream rather than via a second positional read. Verified end to end against a real ALZip ZipCrypto archive (password `1q2w3e4r!` fixture semantics mirrored; fixture itself not imported). AES-128/256 (methods 1/2, WinZip-AES-shaped: salt 10/18 B + 10 B MAC) and LEA-128/256 (methods 5/6) are reported as unsupported by name; no real fixtures exist to verify an implementation against, so none was shipped.
...
- BCJ2 (coder `03 03 01 1B`) is a four-input 7z branch filter Commons Compress structurally cannot decode ("Multi input/output stream coders are not yet supported"); combined with the fact that the bundled native libarchive cannot decrypt 7z, AES-encrypted BCJ2 archives had no path and plain BCJ2 leaned on the native fallback. New `SevenZBcj2ArchiveReader` parses the 7z header itself (start header, encoded/AES-encrypted header via the same decode path, PackInfo/UnpackInfo/SubStreamsInfo, FilesInfo with UTF-16LE names and empty-stream/empty-file vectors), resolves each folder's coder dependency graph (packed streams + bind pairs, final unbound output, cycle-checked), and decodes: Copy pass-through, LZMA/LZMA2 via bundled xz-java, AES-256 via first-party `SevenZAesDecoder` (AES-256-CBC + 7z SHA-256 `salt+UTF16LE(pw)+counter` ×2^power schedule, JCE only), and BCJ2 via first-party `SevenZBcj2Decoder` (range-coded control stream selecting `E8`/`E9`/`0F 8x` conversions; absolute call/jump targets folded back to PC-relative). Wired as a fallback in the 7z list/extract-dir/extract-single paths, gated by `archiveUsesBcj2` so only BCJ2 archives are intercepted - LZMA/LZMA2/PPMd/BZip2/plain-AES 7z is untouched. Clean-room from the documented format and published algorithms (no 7-Zip/libarchive source). Validated black-box against the reference 7z: self-made deterministic fixtures (random bytes interleaved with real x86 `E8`/`E9`/`0F 8x` + 4-byte operands, 2,868 B) in three chains - BCJ2/stored, BCJ2/LZMA, and AES+LZMA+BCJ2 with `-mhe=on` encrypted header - all decode byte-identically; wrong/missing password fails clean. The BCJ2 transform was prototyped in Python against the oracle to pin the algorithm, then ported and re-verified in Java. PPMd stays on the existing native path (AES+PPMd remains unsupported: no bundled backend both decrypts 7z and decodes PPMd). See `docs/SEVENZ_BCJ2_READER_READWIDE_1_0_11.md`.
