...
#define WINZIP_JPEG_MAX_METADATA_BYTES (64U * 1024U * 1024U)
#define WINZIP_JPEG_MAX_SLICE_BYTES (256U * 1024U * 1024U)
...
static inline uint32_t LittleEndianUInt32(uint8_t *ptr)
{
...
int ReadNextWinZipJPEGBundle(WinZipJPEGDecompressor *self)
{
	...
	if(uncompressedsize==0 || uncompressedsize>WINZIP_JPEG_MAX_METADATA_BYTES ||
	compressedsize>WINZIP_JPEG_MAX_METADATA_BYTES) return WinZipJPEGParseError;
	...
	if(compressedsize)
	{
		...
		if(res!=SZ_OK || destlen!=uncompressedsize) return WinZipJPEGLZMAError;
	}
	else
	{
	...
	{
		size_t blockcount=self->jpeg.horizontalmcus;
		unsigned int factors[3]={self->sliceheight,
		self->jpeg.scancomponents[i].component->horizontalfactor,
		self->jpeg.scancomponents[i].component->verticalfactor};
		for(int j=0;j<3;j++)
		{
			if(factors[j]==0 || blockcount>SIZE_MAX/factors[j]) return WinZipJPEGParseError;
			blockcount*=factors[j];
		}
		if(blockcount>WINZIP_JPEG_MAX_SLICE_BYTES/sizeof(WinZipJPEGBlock))
		return WinZipJPEGOutOfMemoryError;
		self->blocks[i]=malloc(blockcount*sizeof(WinZipJPEGBlock));
