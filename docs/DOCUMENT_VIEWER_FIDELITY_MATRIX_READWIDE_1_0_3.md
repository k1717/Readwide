...
| HWPX | L1 text-first fallback | L3 content-fidelity HTML after DOCX | HWPX text extraction exists, and the rendered path now carries header/run styles, page metrics, and table color where directly present. Broad OWPML layout/style/table/image fidelity is still not claimed. |
| HWP 5.x | L1 text-first fallback | Partial L3 after HWPX | HWP binary now converts section/paragraph/control structure into the rendered model: tables with column spans, proportional widths, per-edge cell borders, and authored cell heights; character size/bold/italic/color and underline; paragraph alignment; paragraph-head bullet markers; empty-paragraph spacing; and GSO control lines as horizontal rules. The official-letter heuristic remains only as a fallback. Cell vertical alignment, cell background fill, non-line GSO shapes (rectangles/images), and encrypted/password HWP are not yet claimed. |
...
| HWPX L3 v0 | OWPML paragraphs/runs/tables/images | partial; header/run styles, page metrics, and table color connected behind fallback |
| HWP 5.x partial L3 | char/para shape, table, BinData image | partial; binary section/paragraph/control structure, tables with spans/widths/per-edge borders/cell heights, char and paragraph styles, bullet heads, and control-line horizontal rules connected; BinData images not started |
...
- HWPX rendered output carries header/run styles, page metrics, and table color where directly present; broad OWPML table/image fidelity is not yet claimed.
