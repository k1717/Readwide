...
- **RAR5 compressed decoding:** first-party decoder
  (`Rar5CompressedDecoder` + `Rar5CompressedArchiveExtractor`) for unencrypted,
  single-volume RAR5 entries using compression algorithm version 5.0
  (methods 1-5, dictionaries up to 64 MB), including solid runs with window
  carryover and predecessor priming, and the DELTA / x86 / ARM filters. Every
  entry is CRC32-verified; non-5.0 algorithm versions (RAR7-era streams),
  encrypted, and split cases fail cleanly. This path runs only after the
  libarchive-primary attempt has failed or is unavailable.
...
- RAR5 encrypted or encrypted-header extraction, RAR5 algorithm versions other than 5.0 (RAR7-era streams), and RAR5 dictionaries above 64 MB;
...
> RAR/CBR support is limited. Stored RAR entries, unencrypted single-volume RAR3/RAR4 PPMd entries (including PPMd solid comic archives), and unencrypted single-volume RAR5 v5.0 compressed entries (including solid runs) have first-party handling with CRC verification; other common compressed RAR entries are attempted through bundled libarchive-android. Split, encrypted, classic-LZ compressed-solid RAR3/RAR4, broad SFX, VM-filtered, RAR7-era algorithm versions, and encrypted-header cases are not guaranteed.
