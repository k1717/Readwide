...
Readwide applies the existing 32 GiB extraction safety limit while streaming data
from generic decoded archive streams. This now covers:
...
If the decoded stream crosses the safety limit, extraction fails with an explicit
...
unknown-size decoded streams from bypassing the existing extraction size guard.

## Still intentionally limited

- Unsupported codecs remain unsupported.
- Encrypted entries still require a password or fail according to the backend.
- This is not zip-bomb detection by compression ratio; it is a hard decoded-byte
  ceiling for stream extraction.
